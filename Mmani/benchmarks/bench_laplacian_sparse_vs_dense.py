...
TODO: generate the data -- what should it be
      precompute -- does it make sense to add? - i'll write a different function maybe
...
def compute_bench(n_samples, n_features, rad0, dim, precompute):
    ...
    for ns in n_samples:
        ...
        for nf in n_features:
            ...
            rad = rad0/ns**(1./(2*dim+4))  #check the scaling
            print( 'rad=', rad, 'ns=', ns )
            if nf < 3:
...
if __name__ == '__main__':
    ...
    dim = 2
    n_features = 10
    list_n_samples = np.linspace(500, 10000, 2).astype(np.int)
    sparse_d_results, sparse_a_results, sparse_l_results,  dense_d_results, dense_a_results, dense_l_results = compute_bench(list_n_samples,
                                            [n_features], rad0, dim, precompute=False)

    save_dict = { 'ns_sparse_d_results':sparse_d_results, 'ns_sparse_a_results':sparse_a_results,'ns_sparse_l_results':sparse_l_results, 'ns_dense_d_results':dense_d_results, 'ns_dense_a_results':dense_a_results, 'ns_dense_l_results':dense_l_results }
    if is_save:
    ...
    n_samples = 2000
    list_n_features = np.linspace(5, 1000, 2).astype(np.int)
    sparse_d_results, sparse_a_results, sparse_l_results, dense_d_results, dense_a_results, dense_l_results = compute_bench([n_samples], list_n_features, rad0, dim, precompute=False)
    nf_dict = {'nf_sparse_d_results':sparse_d_results, 'nf_sparse_a_results':sparse_a_results, 'nf_sparse_l_results':sparse_l_results, 'nf_dense_d_results':dense_d_results, 'nf_dense_a_results':dense_a_results, 'nf_dense_l_results':dense_l_results }
